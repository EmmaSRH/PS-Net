...
class PerceptualHausdorfffLoss(nn.Module):
    ...
    def forward(self, set1, set2, tolerance=2):
        ...
        set1, set2 = get_points(set1), get_points(set2)
        ...
        # res = averaged_hausdorff_distance(set1, set2)
        # print(res)
...
class WeightedHausdorffDistance(nn.Module):
    ...
    def forward(self, prob_map, gt, orig_sizes):
        ...
        # _assert_no_grad(gt)
        prob_map = torch.squeeze(prob_map,1)
        self.device = prob_map.device
        gt = [torch.from_numpy(np.argwhere(cv2.imread(gt_pt_path,0)>0)).to(self.device) \
                 for gt_pt_path in  gt]
        ...
        for b in range(batch_size):
            ...
            orig_size_b = orig_sizes # each original size of batch is same
