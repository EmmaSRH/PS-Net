...
class Solver(object):
    ...
    def build_model(self):
        ...
        # self.unet = torch.nn.parallel.DistributedDataParallel(self.unet, device_ids=self.gpus)
        self.unet.cuda()
        ...
        if os.path.isfile(unet_path):
            ...
            for epoch in range(self.num_epochs):
                ...
                for i, (filename, image, GT, img_crop_list, gt_crop_list) in enumerate(tqdm(self.train_loader)):
                    ...
                    acc += get_accuracy(pre, GT)
                    SE += get_sensitivity(pre, GT)
                    SP += get_specificity(pre, GT)
                    PC += get_precision(pre, GT)
                    F1 += get_F1(pre, GT)
                    PHD += phd
                    length += 1
                ...
                # Print the log info
                print(
                    'Epoch [%d/%d], Loss: %.4f, \n[Training] Acc: %.4f, SE: %.4f, SP: %.4f, PC: %.4f, F1: %.4f, PHD: %.4f' % (
                        epoch + 1, self.num_epochs, epoch_loss, acc, SE, SP, PC, F1, PHD))
                ...
                with torch.no_grad():
                    ...
                    print('[Validation] Acc: %.4f, SE: %.4f, SP: %.4f, PC: %.4f, F1: %.4f, PHD: %.4f' % (
                    acc, SE, SP, PC, F1, PHD))
                    # unet_score = F1
                    ...
                    wr = csv.writer(f)
                    wr.writerow(
                        [self.model_type, acc, SE, SP, PC, F1, self.lr, self.num_epochs, self.num_epochs_decay,
